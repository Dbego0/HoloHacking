...
(function () {
  ...
  estilo.textContent = [
    ...
    "  line-height:1.5;max-width:34ch;text-shadow:0 2px 20px rgba(0,0,0,.65)}",
  ...
  function rolarAte(sel, p, ini, fim) {
    ...
    window.scrollTo(0, base + (alvo - base) * suave(p, ini, fim));
  }
  ...
  function catalogo(modulo, titulo) {
    return {
      dur: 10,
      legenda: titulo + " · dez ferramentas",
      entrar: function () {
        mostrarPainel(false);
        irSecao(modulo);
        window.scrollTo(0, 0);
      },
      quadro: function (p) { window.scrollTo(0, 360 * suave(p, .22, .95)); }
    };
  }
  ...
  var CENAS = [
    ...
    { dur: 7, legenda: "",
      ...
      quadro: function (p) { animarLinhas(p, .06, .34); } },

    /* 3 — o questionario, respondendo de verdade */
    { dur: 13, legenda: "Quinze minutos. Oitenta e sete perguntas que ninguém faz.",
      entrar: function () {
        mostrarPainel(false);
      ...
      quadro: function (p) {
        ...
        window.scrollTo(0, 240 + 520 * suave(p, .25, .95));
      } },

    /* 4 — as perguntas que ninguem faz */
    { dur: 9, legenda: "Nenhuma delas está numa anamnese comum.",
      entrar: function () {
        mostrarPainel(true);
    ...
    { dur: 9, continua: true, legenda: "Corpo, mente e espírito num gráfico só.",
      entrar: function () { mostrarPainel(false); },
      quadro: function (p) { rolarAte("#holo-triada", p, .05, .55); } },

    /* 7 — a virada, app de verdade */
    { dur: 9, continua: true, legenda: "Você não tem falta de disciplina. Você tem falta de diagnóstico.",
      entrar: function () { mostrarPainel(false); },
      quadro: function (p) { rolarAte(".leitura-combinada", p, .05, .5); } },

    /* 8 — por onde comecar, app de verdade */
    { dur: 8, continua: true, legenda: "O mapa não para no diagnóstico. Ele diz por onde começar.",
      entrar: function () { mostrarPainel(false); },
      quadro: function (p) { rolarAte(".leitura-conduta", p, .05, .5); } },

    /* 9 — a ferramenta abre, app de verdade */
    { dur: 10, legenda: "E a ferramenta certa abre com um clique.",
    ...
    { dur: 13, legenda: "Trouxe exame? Ele confronta o relato com o que o sangue mostra.",
      entrar: function () {
        mostrarPainel(false);
        abrirFicha();
      ...
      quadro: function (p) {
        if (p > .4) legendaTxt.textContent =
          "Quando os dois não batem, ali está o que a anamnese não pega.";
        rolarAte("#ex-confronto", p, .15, .6);
      } },

    /* 14 — o relatorio, app de verdade */
    { dur: 12, legenda: "O mapa vira relatório. Um para ela, outro para a paciente.",
      entrar: function () {
        mostrarPainel(false);
      ...
      quadro: function (p) {
        window.scrollTo(0, 420 * suave(p, .1, .55));
    ...
    { dur: 11, legenda: "E a ficha avisa o que ninguém estava vendo.",
      entrar: function () {
        mostrarPainel(false);
        abrirFicha();
        window.scrollTo(0, 0);
    ...
    { dur: 12, legenda: "Doze semanas depois, com o mesmo questionário.",
      entrar: function () {
        mostrarPainel(false);
        prepararEvolucao();
        irSecao("holoscope");
